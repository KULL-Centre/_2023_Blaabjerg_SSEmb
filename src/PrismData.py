...
class VariantData(PrismData):
    ...
    def to_new_reference(self, target_seq=None, first_resn=None, verbose=0,
                         min_identity=.6, min_coverage=.1, mismatch="remove", allow_inserts=True, allow_deletions=True):
        ...
        if not target_seq is None:
            ...
            if len(align) > 1:
                ...
                if any(align_scores[0] - align_scores[1:] < 1e-3):
                    ...
                    print("WARNING: There are alignments for %s with same or better scores")
            # Use first alignment
            align = align[0]
